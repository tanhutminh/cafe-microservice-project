...
Some modules are the exception: certain test classes run against a real Postgres container via [Testcontainers](https://testcontainers.com/) rather than a mock, to actually exercise behavior — a real lock serializing concurrent transactions, real Hibernate persistence-context state, custom JPQL/SQL — a mocked repository can't verify. order-service and inventory-service currently have such tests, so `mvn -pl <module> -am test` for either needs a reachable Docker daemon; each of those classes is tagged `@Tag("testcontainers")` so it can be excluded (`-DexcludedGroups=testcontainers`) in contexts without one, e.g. that service's own Docker image build stage. Every service's Docker build runs its unit test suite as part of `mvn package` (no `-DskipTests` anywhere), so `docker compose build <service>` doubles as a test gate, not just a packaging step.
...
Một số module là ngoại lệ: 1 số class test chạy trên Postgres thật qua [Testcontainers](https://testcontainers.com/) thay vì mock, để thực sự kiểm chứng các hành vi mà 1 repository bị mock không thể kiểm tra được — 1 khoá thật giúp serialize các transaction chạy đồng thời, trạng thái persistence-context thật của Hibernate, hay JPQL/SQL tự viết. order-service và inventory-service hiện đều có loại test này, nên `mvn -pl <module> -am test` với module nào trong 2 module đó cũng cần Docker daemon đang chạy; mỗi class như vậy được gắn `@Tag("testcontainers")` để có thể loại trừ (`-DexcludedGroups=testcontainers`) ở những nơi không có Docker, ví dụ bước build Docker image của chính service đó. Docker build của mọi service đều chạy unit test như 1 phần của `mvn package` (không có `-DskipTests` ở đâu cả), nên `docker compose build <service>` vừa build vừa đóng vai trò 1 cổng kiểm thử, không chỉ đơn thuần là đóng gói.
