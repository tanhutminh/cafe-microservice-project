...
- **Dead Letter Queue** — inventory-service and order-service each route messages that fail for *technical* reasons at the Kafka-receipt layer (bad payload, bugs, DB errors — never a business "insufficient stock" outcome, which is a normal reply, not an exception) to a `.dlq` topic after a short exponential-backoff retry, instead of blocking the consumer on a poison-pill message. Applies uniformly to all three inventory command topics (`inventory.reserve-stock.command`, `inventory.commit-stock.command`, `inventory.release-stock.command`) and to order-service's two reply topics (`inventory.stock-reservation.reply`, `inventory.stock-commit.reply`), each service via one shared error-handler bean, not configured per topic
