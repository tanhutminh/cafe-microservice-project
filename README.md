...
- **Truy vết phân tán (Distributed Tracing)** — mọi service đều export span sang Zipkin (`http://localhost:9411`) qua Micrometer Tracing + Brave; HTTP (routing ở gateway, các lời gọi WebClient) và Kafka produce/consume được tự động instrument (`spring.kafka.template`/`listener.observation-enabled`), nên `traceId` của 1 request sống sót qua mọi hop mạng mà không cần code thêm gì.
  ...
  - Health-check polling của Docker (`GET /actuator/health`, gọi mỗi vài giây/container) bị loại khỏi tracing ở mọi service. Các poller định kỳ riêng của từng service cũng bị loại tương tự — `OutboxPoller` và `OrderSagaReconciliationJob` của order-service; `OutboxPoller` và `InboxPoller` của inventory-service (2 service duy nhất có method `@Scheduled`). Nếu không lọc, cả 2 nguồn này sẽ làm ngập Zipkin bằng những span lặp lại không mang thông tin gì mới, chôn vùi trace của request thật bên dưới. Bean `ObservabilityConfig` của mỗi service match theo field riêng của đúng loại `Observation.Context` — path pattern của health-check, hoặc target class của scheduled task — không bao giờ match theo *tên* observation dùng chung với mọi request/task khác (`http.server.requests` cho mọi HTTP request, `tasks.scheduled.execution` cho mọi method `@Scheduled`). Lọc theo tên sẽ âm thầm tắt tracing của mọi endpoint hoặc mọi scheduled method tương lai, không chỉ riêng mấy cái này.
