...
Since this project's purpose is to practice canonical patterns, worth calling out explicitly which ones are implemented so far, grouped by what problem they solve rather than by when they were added. Names follow the common catalogs. Chris Richardson's [microservices.io](https://microservices.io/patterns/index.html) covers most of what's below — API Gateway, Service Discovery, Externalized Configuration, Database per Service, Circuit Breaker, Saga, Transactional Outbox (with its Polling publisher), Idempotent Consumer, Health Check API, Distributed Tracing — and the rest come from elsewhere: Dead Letter Queue is *Enterprise Integration Patterns*' Dead Letter Channel, Retry and Reconciliation are closest to Azure's Cloud Design Patterns (Retry, Scheduler Agent Supervisor), Try-Confirm/Cancel is Guy Pardon's, Atomic Conditional Update is the lightweight cousin of Optimistic Concurrency Control (Kung & Robinson, 1981; Fowler's Optimistic Offline Lock), and Transactional Inbox and Trusted Header Authentication are descriptive names with no single catalog entry. Worth looking up the canonical definition first if a name is unfamiliar, then coming back to see how this codebase applies it.

### Platform

- **Service Discovery** — ~~Eureka (`eureka-server`)~~ **replaced 2026-09** by Kubernetes Service DNS; see [Retired components](#retired-components)
- **API Gateway** — Spring Cloud Gateway, single entry point + CORS + routing
- **Externalized Configuration** — ~~Spring Cloud Config Server, native profile backed by a bind-mounted `config-repo`~~ **replaced 2026-09** by ConfigMap + Secret; see [Retired components](#retired-components)
- **Trusted Header Authentication** — gateway validates the JWT once and forwards identity via `X-User-Id`/`X-Username`/`X-User-Role` headers; downstream services trust the gateway instead of re-validating (`common-lib`'s `HeaderAuthenticationFilter`, registered in the `SecurityConfig` of each API-exposing service; `TrustedHeaderAuth` only documents the same headers for Swagger UI)
- **Database per Service** — separate Postgres database and role per service

### Resilience

- **Circuit Breaker + Retry** — Resilience4j on order-service's calls to menu-service. Circuit Breaker is Nygard's (*Release It!*, 2007); Retry is its standard companion, catalogued by Azure rather than by microservices.io
...
- **Dead Letter Queue** — inventory-service and order-service each route messages that fail for *technical* reasons at the Kafka-receipt layer (bad payload, bugs, DB errors — never a business "insufficient stock" outcome: no stock logic runs at that layer, and the outcome comes back as a normal reply, not an exception) to a `.dlq` topic after a short exponential-backoff retry, instead of blocking the consumer on a poison-pill message. Applies uniformly to all three inventory command topics (`inventory.reserve-stock.command`, `inventory.commit-stock.command`, `inventory.release-stock.command`) and to order-service's two reply topics (`inventory.stock-reservation.reply`, `inventory.stock-commit.reply`), each service via one shared error-handler bean, not configured per topic

### Observability

- **Health Check API** — every service exposes Spring Boot Actuator health endpoints. The Helm chart wires its `startupProbe`/`readinessProbe`/`livenessProbe` to the separate `/actuator/health/liveness` and `/actuator/health/readiness` groups (`charts/cafe-service/templates/deployment.yaml`), so a transient DB blip fails readiness without also failing liveness; under `docker compose`, container health checks poll `/actuator/health`.
- **Distributed Tracing** — under `docker compose`, every service exports spans to the `zipkin` container (browsable at `http://localhost:9411`) via Micrometer Tracing + Brave; HTTP (gateway routing, WebClient calls) and Kafka produce/consume are auto-instrumented (`spring.kafka.template`/`listener.observation-enabled`), so a request's `traceId` survives every network hop for free. The Kubernetes deployment has no tracing backend yet — the Helm chart sets no exporter endpoint and deploys no Zipkin.
...
Vì mục đích của dự án là luyện tập các pattern kinh điển, nên liệt kê rõ những pattern nào đã được áp dụng tính tới thời điểm hiện tại, nhóm theo vấn đề chúng giải quyết thay vì theo thứ tự implement. Tên pattern theo đúng các catalog phổ biến. Bộ [microservices.io](https://microservices.io/patterns/index.html) của Chris Richardson bao phủ phần lớn những pattern dưới đây — API Gateway, Service Discovery, Externalized Configuration, Database per Service, Circuit Breaker, Saga, Transactional Outbox (cùng Polling publisher của nó), Idempotent Consumer, Health Check API, Distributed Tracing — số còn lại đến từ nguồn khác: Dead Letter Queue chính là Dead Letter Channel của *Enterprise Integration Patterns*; Retry và Reconciliation gần nhất với catalog Cloud Design Patterns của Azure (Retry, Scheduler Agent Supervisor); Try-Confirm/Cancel là của Guy Pardon; Atomic Conditional Update là họ hàng nhẹ của Optimistic Concurrency Control (Kung & Robinson, 1981; Optimistic Offline Lock của Fowler); còn Transactional Inbox và Trusted Header Authentication là tên mô tả, không có mục nào riêng trong catalog nào. Nên tra định nghĩa gốc trước nếu chưa quen tên, rồi quay lại xem codebase này áp dụng nó thế nào.

### Nền tảng (Platform)

- **Service Discovery** — ~~Eureka (`eureka-server`)~~ **đã thay 2026-09** bằng Service DNS của Kubernetes; xem mục [Thành phần đã retired](#thành-phần-đã-retired)
- **API Gateway** — Spring Cloud Gateway, cổng vào duy nhất + CORS + routing
- **Externalized Configuration** — ~~Spring Cloud Config Server, profile native được backing bởi `config-repo` bind-mount~~ **đã thay 2026-09** bằng ConfigMap + Secret; xem mục [Thành phần đã retired](#thành-phần-đã-retired)
- **Trusted Header Authentication** — gateway xác thực JWT một lần duy nhất rồi chuyển tiếp danh tính qua header `X-User-Id`/`X-Username`/`X-User-Role`; các service phía sau tin tưởng gateway thay vì tự xác thực lại (`HeaderAuthenticationFilter` trong `common-lib`, đăng ký trong `SecurityConfig` của từng service có expose API; `TrustedHeaderAuth` chỉ mô tả các header đó cho Swagger UI)
- **Database per Service** — mỗi service có 1 database Postgres và 1 role riêng

### Khả năng chịu lỗi (Resilience)

- **Circuit Breaker + Retry** — Resilience4j cho lời gọi từ order-service sang menu-service. Circuit Breaker là của Nygard (*Release It!*, 2007); Retry là pattern đi kèm kinh điển của nó, nằm trong catalog của Azure chứ không phải microservices.io

### Saga đơn hàng & tính nhất quán

- **Orchestrated Saga** — luồng checkout của order-service điều khiển một state machine (`OrderSaga`) gồm 2 chặng: Xác thực (giữ chỗ mềm tồn kho, `OPEN`→`CONFIRMED`) và Thanh toán (commit chỗ đã giữ, `CONFIRMED`→`PAID`), mỗi chặng là 1 vòng round-trip Kafka riêng, tự commit hoặc compensate dựa theo reply nhận được; xem mục [Luồng nghiệp vụ: saga xác thực và thanh toán](#luồng-nghiệp-vụ-saga-xác-thực-và-thanh-toán) bên trên
...
- **Atomic Conditional Update (compare-and-set)** — họ hàng nhẹ của Optimistic Concurrency Control: không có cột version hay bản snapshot đã đọc để kiểm tra lại lúc commit; việc kiểm tra được gộp thẳng vào chính câu lệnh ghi. `DiningTableService.occupy()`/`release()` mỗi hàm tự bảo vệ 1 bàn bằng 1 câu lệnh `UPDATE ... WHERE` có điều kiện duy nhất (`DiningTableRepository.occupyIfAvailable`/`releaseIfAllOrdersClosed`) thay vì đọc-rồi-ghi tách rời. `WHERE` của `occupy()` check status của chính bàn đó, nên 2 lần gọi `occupy()` gần như đồng thời trên cùng 1 bàn không thể cùng thành công. `WHERE` của `release()` check (qua subquery) rằng không còn đơn hàng nào gắn với bàn đó ở status non-closed — đóng đúng race giữa việc release và 1 đơn hàng chuyển sang status non-closed ngay giữa chừng, chứ không phải race giữa 2 lần gọi `release()` với nhau (2 lần đó thì cứ thành công vô hại cả 2). Dù cách nào thì cũng là race trên 1 dòng dữ liệu, khác với việc điều phối xuyên service của saga/TCC — 1 rủi ro nhất quán khác, xử lý bằng 1 kỹ thuật khác
...
- **Dead Letter Queue** — inventory-service và order-service mỗi bên chuyển các message lỗi vì nguyên nhân *kỹ thuật* ở tầng nhận message từ Kafka (payload sai định dạng, bug, lỗi DB — không bao giờ tính trường hợp nghiệp vụ "hết hàng": ở tầng đó chưa có logic kho nào chạy, và kết quả quay về như 1 reply bình thường, không phải exception) sang topic `.dlq` sau vài lần retry theo exponential backoff, thay vì để nó chặn cứng consumer (poison-pill message). Áp dụng đồng loạt cho cả 3 topic command của inventory (`inventory.reserve-stock.command`, `inventory.commit-stock.command`, `inventory.release-stock.command`) và 2 topic reply của order-service (`inventory.stock-reservation.reply`, `inventory.stock-commit.reply`), mỗi service qua 1 bean xử lý lỗi dùng chung, không cấu hình riêng từng topic

### Khả năng quan sát (Observability)

- **Health Check API** — mọi service đều expose các endpoint health của Spring Boot Actuator. Helm chart nối `startupProbe`/`readinessProbe`/`livenessProbe` với 2 nhóm health riêng biệt `/actuator/health/liveness` và `/actuator/health/readiness` (`charts/cafe-service/templates/deployment.yaml`), nên 1 lần DB chập chờn thoáng qua làm hỏng readiness mà không làm hỏng luôn liveness; khi chạy `docker compose`, health check của container gọi `/actuator/health`.
- **Truy vết phân tán (Distributed Tracing)** — khi chạy `docker compose`, mọi service export span sang container `zipkin` (xem ở `http://localhost:9411`) qua Micrometer Tracing + Brave; HTTP (routing ở gateway, các lời gọi WebClient) và Kafka produce/consume được tự động instrument (`spring.kafka.template`/`listener.observation-enabled`), nên `traceId` của 1 request sống sót qua mọi hop mạng mà không cần code thêm gì. Bản deploy Kubernetes chưa có backend tracing nào — Helm chart không set endpoint export và cũng không deploy Zipkin.
