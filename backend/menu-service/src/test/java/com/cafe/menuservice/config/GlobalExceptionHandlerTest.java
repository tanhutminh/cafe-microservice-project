...
class GlobalExceptionHandlerTest {
    ...
    void setUp() {
        ...
        when(request.getRequestURI()).thenReturn("/api/menu-items/1");
    }
        ...
        return Stream.of(
                Arguments.of(
                        ...
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
                                (h, req) -> h.handleNotFound(ResourceNotFoundException.of("MenuItem", 1L), req),
                        HttpStatus.NOT_FOUND, "Not Found", "MenuItem not found: 1"
                ),
                Arguments.of(
                        "handleBusinessRule",
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
                                (h, req) -> h.handleBusinessRule(new BusinessRuleException("Category has active menu items"), req),
                        HttpStatus.CONFLICT, "Conflict", "Category has active menu items"
                ),
                Arguments.of(
                        "handleNoResource",
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
    ...
    void simpleHandler_mapsExceptionToExpectedApiError(
        ...
        assertAll(handlerName,
                ...
                () -> assertThat(body.path()).isEqualTo("/api/menu-items/1"),
    ...
    void handleValidation_extractsFieldErrorsFromBindingResult() {
        FieldError fieldError = new FieldError("menuItemRequest", "categoryId", "must not be null");
        ...
        assertAll(
                ...
                () -> assertThat(body.validationErrors())
                        .containsExactly(new ApiError.FieldViolation("categoryId", "must not be null"))
        );
    }
    ...
    private static Stream<Arguments> methodValidationResults() {
        return Stream.of(
                ...
                Arguments.of(
                        ...
                        (Supplier<ParameterValidationResult>) () -> {
                            FieldError fieldError = new FieldError("categoryRequest", "displayOrder", "must be greater than or equal to 0");
                            ParameterErrors bodyErrors = mock(ParameterErrors.class);
                            when(bodyErrors.getFieldErrors()).thenReturn(List.of(fieldError));
                            return bodyErrors;
                        },
                        new ApiError.FieldViolation("displayOrder", "must be greater than or equal to 0")
                )
        );
    }
