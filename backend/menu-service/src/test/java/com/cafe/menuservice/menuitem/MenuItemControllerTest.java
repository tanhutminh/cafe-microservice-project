...
class MenuItemControllerTest {
    ...
    private static final String VALID_BODY = "{\"categoryId\":1,\"name\":\"Cappuccino\",\"price\":45000,\"available\":true,\"active\":true}";
    ...
    private MenuItem sampleItem() {
        Category category = Category.builder().id(1L).name("Coffee").displayOrder(1).active(true).build();
        return MenuItem.builder().id(12L).category(category).name("Cappuccino").description("Foamy")
                .price(BigDecimal.valueOf(45000)).imageUrl(null).available(true).active(true).build();
    }
    ...
    private static Stream<Arguments> validationFailures() {
        return Stream.of(
                Arguments.of("findById_negativeId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> get("/api/menu-items/-1"),
                        "id", "must be greater than 0"),
                Arguments.of("search_negativeCategoryId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> get("/api/menu-items").param("categoryId", "-1"),
                        "categoryId", "must be greater than 0"),
                Arguments.of("create_missingCategoryId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/menu-items").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"name\":\"Cappuccino\",\"price\":45000,\"available\":true,\"active\":true}"),
                        "categoryId", "must not be null"),
                Arguments.of("create_negativeCategoryId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/menu-items").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"categoryId\":-1,\"name\":\"Cappuccino\",\"price\":45000,\"available\":true,\"active\":true}"),
                        "categoryId", "must be greater than 0"),
                Arguments.of("create_blankName",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/menu-items").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"categoryId\":1,\"name\":\"\",\"price\":45000,\"available\":true,\"active\":true}"),
                        "name", "must not be blank"),
                Arguments.of("create_descriptionTooLong",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/menu-items").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"categoryId\":1,\"name\":\"Cappuccino\",\"description\":\"" + "a".repeat(2001)
                                        + "\",\"price\":45000,\"available\":true,\"active\":true}"),
                        "description", "size must be between 0 and 2000"),
                Arguments.of("create_missingPrice",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/menu-items").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"categoryId\":1,\"name\":\"Cappuccino\",\"available\":true,\"active\":true}"),
                        "price", "must not be null"),
                Arguments.of("create_negativePrice",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/menu-items").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"categoryId\":1,\"name\":\"Cappuccino\",\"price\":-1,\"available\":true,\"active\":true}"),
                        "price", "must be greater than or equal to 0"),
                Arguments.of("create_imageUrlTooLong",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/menu-items").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"categoryId\":1,\"name\":\"Cappuccino\",\"price\":45000,\"imageUrl\":\"" + "a".repeat(501)
                                        + "\",\"available\":true,\"active\":true}"),
                        "imageUrl", "size must be between 0 and 500"),
                Arguments.of("updateAvailability_missingAvailableField",
                        (Supplier<MockHttpServletRequestBuilder>) () -> patch("/api/menu-items/12/availability")
    ...
    void findById_nonConvertibleId_returns400NotUnexpected500(String caseName, String idSegment) throws Exception {
        ...
        mockMvc.perform(get("/api/menu-items/" + idSegment)
                        .header("X-Username", "admin")
                        .header("X-User-Role", "ADMIN"))
                .andExpect(status().isBadRequest())
    ...
    void search_returns200WithMenuItemList() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/api/menu-items")
                        .header("X-Username", "admin")
                        .header("X-User-Role", "ADMIN"))
    ...
    private static Stream<Arguments> happyPathRequests() {
        return Stream.of(
                Arguments.of("findById",
                        (Supplier<MockHttpServletRequestBuilder>) () -> get("/api/menu-items/12"), HttpStatus.OK),
                Arguments.of("create",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/menu-items")
                                .contentType(MediaType.APPLICATION_JSON).content(VALID_BODY),
                        HttpStatus.CREATED),
                Arguments.of("update",
                        (Supplier<MockHttpServletRequestBuilder>) () -> put("/api/menu-items/12")
                                .contentType(MediaType.APPLICATION_JSON).content(VALID_BODY),
                        HttpStatus.OK),
                Arguments.of("updateAvailability",
                        (Supplier<MockHttpServletRequestBuilder>) () -> patch("/api/menu-items/12/availability")
    ...
    void validRequest_returnsExpectedStatusWithMenuItem(
        ...
        switch (caseName) {
            case "findById" -> when(menuItemService.findById(12L)).thenReturn(sampleItem());
            ...
            case "updateAvailability" -> when(menuItemService.updateAvailability(12L, false)).thenReturn(sampleItem());
        }
