...
public class MenuItemController {
  ...
  public List<MenuItemResponse> search(
      @Parameter(description = "Only return items in this category", example = "1")
          @RequestParam(required = false)
          @Positive
          Long categoryId,
      @Parameter(description = "Only return items with this availability", example = "true")
          @RequestParam(required = false)
          Boolean available) {
    return menuItemService.search(categoryId, available).stream()
  ...
  @GetMapping("/batch")
  @Operation(
      summary = "Fetch multiple menu items by id in one request, instead of one request per id")
  public List<MenuItemResponse> findAllById(
      @Parameter(description = "The menu item ids to fetch", example = "[1,2,3]")
          @RequestParam
          @Size(max = 50)
          List<@Positive Long> ids) {
    return menuItemService.findAllById(ids).stream().map(MenuItemResponse::from).toList();
  }
  ...
  public MenuItemResponse update(
      @Parameter(description = "The menu item's id", example = "12") @PathVariable @Positive
          Long id,
  ...
  @Operation(
      ...
      description =
          "Toggles availability without deleting the item. \"86\" is F&B slang for "
