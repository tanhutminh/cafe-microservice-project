...
class UserControllerTest {
  ...
  void updateStatus_invalidRequest_returns400WithFieldViolation(
      String caseName, String url, String body, String expectedField) throws Exception {
    mockMvc
        .perform(
            patch(url)
  ...
  private static Stream<Arguments> updateStatusAuthFailures() {
    return Stream.of(
        Arguments.of(
            "wrongRole_cashier",
            Map.of("X-Username", "cashier", "X-User-Role", "CASHIER"),
  ...
  void updateStatus_insufficientAuth_returnsExpectedStatus(
      String caseName, Map<String, String> headers, HttpStatus expectedStatus) throws Exception {
  ...
  void updateStatus_nonConvertibleId_returns400NotUnexpected500(String caseName, String idSegment)
      throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/users/" + idSegment + "/status")
                .header("X-Username", "admin")
                .header("X-User-Role", "ADMIN")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"active\":false}"))
  ...
  void findAll_returns200WithUserList() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(get("/api/users").header("X-Username", "admin").header("X-User-Role", "ADMIN"))
            .andExpect(status().isOk())
            .andReturn();
  ...
  void create_validRequest_returns201WithUser() throws Exception {
    when(userService.create("cashier2", "a-strong-password", "Nguyen Van A", Role.CASHIER))
        ...
        mockMvc
            .perform(
                post("/api/users")
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        "{\"username\":\"cashier2\",\"password\":\"a-strong-password\",\"fullName\":\"Nguyen Van A\",\"role\":\"CASHIER\"}"))
  ...
  private void assertUserResponseBody(
      MvcResult result, long id, String username, String fullName, String role, boolean active)
      throws Exception {
