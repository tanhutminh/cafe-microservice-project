...
class GlobalExceptionHandlerTest {
        ...
        return Stream.of(
                Arguments.of(
                        ...
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
                                (h, req) -> h.handleNotFound(ResourceNotFoundException.of("User", 1L), req),
                        HttpStatus.NOT_FOUND, "Not Found", "User not found: 1"
                ),
                Arguments.of(
                        "handleBusinessRule",
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
                                (h, req) -> h.handleBusinessRule(new BusinessRuleException("Username already taken"), req),
                        HttpStatus.CONFLICT, "Conflict", "Username already taken"
                ),
                Arguments.of(
                        "handleInvalidCredentials",
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
                                (h, req) -> h.handleInvalidCredentials(new InvalidCredentialsException("Bad username or password"), req),
                        HttpStatus.UNAUTHORIZED, "Unauthorized", "Bad username or password"
                ),
                Arguments.of(
                        "handleNoResource",
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
    ...
    void handleValidation_extractsFieldErrorsFromBindingResult() {
        FieldError fieldError = new FieldError("createUserRequest", "username", "must not be blank");
        ...
        assertAll(
                ...
                () -> assertThat(body.validationErrors())
                        .containsExactly(new ApiError.FieldViolation("username", "must not be blank"))
        );
    }
    ...
    private static Stream<Arguments> methodValidationResults() {
        return Stream.of(
                ...
                Arguments.of(
                        ...
                        (Supplier<ParameterValidationResult>) () -> {
                            FieldError fieldError = new FieldError("updateUserStatusRequest", "active", "must not be null");
                            ParameterErrors bodyErrors = mock(ParameterErrors.class);
                            when(bodyErrors.getFieldErrors()).thenReturn(List.of(fieldError));
                            return bodyErrors;
                        },
                        new ApiError.FieldViolation("active", "must not be null")
                )
        );
    }
