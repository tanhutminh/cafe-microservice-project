...
class RecipeControllerTest {
  ...
  private static Stream<Arguments> validationFailures() {
    return Stream.of(
        Arguments.of(
            "findAllByMenuItemId_negativeId",
            (Supplier<MockHttpServletRequestBuilder>) () -> get("/api/menu-items/-1/recipe"),
            "menuItemId",
            "must be greater than 0"),
        Arguments.of(
            "replace_negativeMenuItemId",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    put("/api/menu-items/-1/recipe")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[]"),
            "menuItemId",
            "must be greater than 0"),
        Arguments.of(
            "replace_missingIngredientIdOnLine",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    put("/api/menu-items/12/recipe")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[{\"quantityRequired\":0.5}]"),
            "ingredientId",
            "must not be null"),
        Arguments.of(
            "replace_negativeIngredientIdOnLine",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    put("/api/menu-items/12/recipe")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[{\"ingredientId\":-1,\"quantityRequired\":0.5}]"),
            "ingredientId",
            "must be greater than 0"),
        Arguments.of(
            "replace_missingQuantityRequiredOnLine",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    put("/api/menu-items/12/recipe")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[{\"ingredientId\":7}]"),
            "quantityRequired",
            "must not be null"),
        Arguments.of(
            "replace_quantityRequiredBelowMinimum",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    put("/api/menu-items/12/recipe")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[{\"ingredientId\":7,\"quantityRequired\":0}]"),
            "quantityRequired",
  ...
  void replace_tooManyLines_returns400WithFieldViolation() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.validationErrors[0].message").value("size must be between 0 and 50"));
  }
  ...
  private static Stream<Arguments> happyPathRequests() {
    return Stream.of(
        Arguments.of(
            "findAllByMenuItemId",
            (Supplier<MockHttpServletRequestBuilder>) () -> get("/api/menu-items/12/recipe")),
        Arguments.of(
            "replace",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    put("/api/menu-items/12/recipe")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[{\"ingredientId\":7,\"quantityRequired\":0.2}]")));
  }

  @ParameterizedTest(name = "{0}")
  @MethodSource("happyPathRequests")
  void validRequest_returns200WithRecipeLines(
      String caseName, Supplier<MockHttpServletRequestBuilder> requestSupplier) throws Exception {
    List<MenuItemIngredient> lines = List.of(sampleRecipeLine());
    if ("findAllByMenuItemId".equals(caseName)) {
      when(recipeService.findAllByMenuItemId(12L)).thenReturn(lines);
    } else {
