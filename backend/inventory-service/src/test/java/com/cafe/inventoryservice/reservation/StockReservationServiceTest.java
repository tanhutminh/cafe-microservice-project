...
class StockReservationServiceTest {
  ...
  void setUp() {
    service =
        new StockReservationService(
            ingredientRepository, menuItemIngredientRepository, stockMovementRepository);
  }
  ...
  void reserve_holdsQuantityWhenStockSufficient() {
    ...
    when(menuItemIngredientRepository.findAllByMenuItemIdIn(List.of(MENU_ITEM_ID)))
    ...
    assertAll(
        ...
        () ->
            assertThat(recipeLine.getIngredient().getReservedQuantity())
  ...
  void commit_deductsCurrentStockReleasesHoldAndRecordsMovement() {
    ...
    assertAll(
        ...
        () -> {
          ...
          assertThat(movementCaptor.getValue().getReferenceId())
    ...
    assertAll(
        () ->
            assertThat(recipeLine.getIngredient().getReservedQuantity())
                .isEqualByComparingTo("0.000"),
        () ->
            assertThat(recipeLine.getIngredient().getCurrentStock())
                .isEqualByComparingTo(currentStockBefore),
