...
public class IngredientService {
  ...
  public IngredientService(
      IngredientRepository ingredientRepository, StockMovementRepository stockMovementRepository) {
   ...
   * query per id, throwing the same exception for any id not found.
   */
  public Map<Long, Ingredient> findAllByIdAsMap(List<Long> ids) {
    Map<Long, Ingredient> found =
        ingredientRepository.findAllById(ids).stream()
            .collect(Collectors.toMap(Ingredient::getId, Function.identity()));
    for (Long id : ids) {
      if (!found.containsKey(id)) {
        throw ResourceNotFoundException.of("Ingredient", id);
      }
    }
    return found;
  }
  ...
  /**
   * Row-locks the single ingredient (reusing the same pessimistic-write query the checkout saga
   * uses) so a stock-in can't race a concurrent saga deduction into a lost update.
   */
  @Transactional
  public Ingredient stockIn(Long id, BigDecimal quantity) {
    Ingredient ingredient =
        ingredientRepository.findAllByIdForUpdate(List.of(id)).stream()
            .findFirst()
            .orElseThrow(() -> ResourceNotFoundException.of("Ingredient", id));
