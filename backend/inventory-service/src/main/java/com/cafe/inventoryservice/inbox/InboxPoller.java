 ...
 * shape as OrderSaga's OrderSagaReconciliationJob sweep. Not itself @Transactional: claimBatch()
 * and processOne()/recordFailure() are separate transactions on InboxMessageProcessor, called
 * through that bean's proxy so each gets its own transaction boundary (self-invocation within one
 * class would silently bypass @Transactional).
 */
@Component
