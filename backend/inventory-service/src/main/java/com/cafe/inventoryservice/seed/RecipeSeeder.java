 ...
 * menu-service's MenuSeeder catalog (27 items), so the checkout saga has real recipes to
 * reserve/commit stock against instead of the "no recipe rows = always in stock" fallback.
 * Mirrors menu-service's MenuSeeder / auth-service's AdminSeeder pattern.
 *
 * Ingredients are seeded additively (only names not already present get created), so it's safe
 * to run against a database that already has some ingredients from before - existing rows and
 * their current_stock/reservedQuantity are never touched. Recipes are only seeded if
 * menu_item_ingredients is completely empty, matching MenuSeeder's "seed once, admin edits
 * after" convention: RecipeEditor lets an admin rewrite any item's recipe afterward, and this
 * seeder must never overwrite that.
 *
 * menuItemId below is a hardcoded 1-27 matching MenuSeeder's MENU list order exactly - the same
 * loose, no-real-FK cross-service coupling the schema comment on menu_item_ingredients already
 * documents, valid only because both seeders run once against an empty table on a fresh
 * database.
 */
@Component
public class RecipeSeeder implements ApplicationRunner {
    ...
    private record SeedIngredient(String name, String unit, BigDecimal initialStock, BigDecimal minStock) {
    }
    ...
    private static final List<SeedIngredient> INGREDIENTS = List.of(
            ...
            new SeedIngredient("Đường trắng", "g", BigDecimal.valueOf(10000), BigDecimal.valueOf(3000)),
            ...
            new SeedIngredient("Sốt caramel", "ml", BigDecimal.valueOf(1000), BigDecimal.valueOf(200)),
            ...
            new SeedIngredient("Phô mai mascarpone", "g", BigDecimal.valueOf(600), BigDecimal.valueOf(100)),
            new SeedIngredient("Bánh mì sandwich", "cái", BigDecimal.valueOf(30), BigDecimal.valueOf(5)),
    ...
    private static final List<SeedRecipe> RECIPES = List.of(
            ...
            new SeedRecipe(4, "Cà phê sữa nóng", List.of(line("Cà phê hạt", 20), line("Sữa đặc", 30))),
            ...
            new SeedRecipe(9, "Trà đào cam sả", List.of(
                    line("Trà đen", 5), line("Đào ngâm", 40), line("Cam tươi", 0.5),
                    line("Sả tươi", 5), line("Đường trắng", 10))),
            ...
            new SeedRecipe(12, "Trà sữa trân châu đường đen", List.of(
                    line("Trà đen", 6), line("Sữa tươi", 100), line("Trân châu", 50), line("Đường đen", 20))),
            new SeedRecipe(13, "Trà sữa matcha", List.of(line("Bột matcha", 10), line("Sữa tươi", 150))),
            ...
            new SeedRecipe(18, "Đá xay matcha", List.of(
                    line("Bột matcha", 12), line("Sữa tươi", 150), line("Kem tươi", 30))),
            new SeedRecipe(19, "Đá xay socola", List.of(
                    line("Bột cacao", 25), line("Sữa tươi", 150), line("Kem tươi", 30))),
            new SeedRecipe(20, "Đá xay caramel", List.of(
                    line("Cà phê hạt", 15), line("Sốt caramel", 30), line("Sữa tươi", 120), line("Kem tươi", 30))),

            new SeedRecipe(21, "Bánh croissant", List.of(line("Bột mì", 80), line("Bơ lạt", 40))),
            new SeedRecipe(22, "Bánh tiramisu", List.of(
                    line("Phô mai mascarpone", 60), line("Cà phê hạt", 10), line("Trứng gà", 1), line("Bột mì", 20))),
            new SeedRecipe(23, "Bánh cheesecake", List.of(
                    line("Phô mai kem", 90), line("Bột mì", 15), line("Trứng gà", 1))),
            new SeedRecipe(24, "Bánh su kem", List.of(
                    line("Bột mì", 30), line("Trứng gà", 1), line("Kem tươi", 50))),

            new SeedRecipe(25, "Sandwich gà", List.of(
                    line("Bánh mì sandwich", 1), line("Ức gà", 80), line("Xà lách", 15), line("Cà chua", 20))),
            new SeedRecipe(26, "Khoai tây chiên", List.of(line("Khoai tây", 200))),
            new SeedRecipe(27, "Bánh mì que pate", List.of(
                    line("Bánh mì que", 1), line("Pate", 30), line("Chả lụa", 20)))
    );
    ...
    public RecipeSeeder(IngredientRepository ingredientRepository, IngredientService ingredientService,
                         MenuItemIngredientRepository menuItemIngredientRepository, RecipeService recipeService) {
    ...
    private Map<String, Ingredient> seedMissingIngredients() {
        Map<String, Ingredient> byName = ingredientRepository.findAll().stream()
    ...
    private void seedRecipesIfEmpty(Map<String, Ingredient> byName) {
        ...
        for (SeedRecipe recipe : RECIPES) {
            List<RecipeItemRequest> lines = recipe.lines().stream()
                    .map(l -> new RecipeItemRequest(byName.get(l.ingredientName()).getId(), l.quantityRequired()))
                    .toList();
