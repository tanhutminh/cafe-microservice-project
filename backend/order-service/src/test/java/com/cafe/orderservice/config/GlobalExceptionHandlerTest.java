...
class GlobalExceptionHandlerTest {
        ...
        return Stream.of(
                Arguments.of(
                        ...
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
                                (h, req) -> h.handleNotFound(ResourceNotFoundException.of("Order", 1L), req),
                        HttpStatus.NOT_FOUND, "Not Found", "Order not found: 1"
                ),
                Arguments.of(
                        "handleBusinessRule",
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
                                (h, req) -> h.handleBusinessRule(new BusinessRuleException("Cannot checkout an empty order"), req),
                        HttpStatus.CONFLICT, "Conflict", "Cannot checkout an empty order"
                ),
                Arguments.of(
                        "handleServiceUnavailable",
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
                                (h, req) -> h.handleServiceUnavailable(
                                        new ServiceUnavailableException("menu-service unreachable", new RuntimeException("connection refused")), req),
                        HttpStatus.SERVICE_UNAVAILABLE, "Service Unavailable", "menu-service unreachable"
                ),
                Arguments.of(
                        "handleNoResource",
                        (BiFunction<GlobalExceptionHandler, HttpServletRequest, ResponseEntity<ApiError>>)
    ...
    void handleValidation_extractsFieldErrorsFromBindingResult() {
        FieldError fieldError = new FieldError("createOrderRequest", "tableId", "must not be null");
        ...
        assertAll(
                ...
                () -> assertThat(body.validationErrors())
                        .containsExactly(new ApiError.FieldViolation("tableId", "must not be null"))
        );
    }
    ...
    private static Stream<Arguments> methodValidationResults() {
        return Stream.of(
                ...
                Arguments.of(
                        ...
                        (Supplier<ParameterValidationResult>) () -> {
                            FieldError fieldError = new FieldError("payRequest", "paymentMethod", "must match \"CASH|CARD\"");
                            ParameterErrors bodyErrors = mock(ParameterErrors.class);
                            when(bodyErrors.getFieldErrors()).thenReturn(List.of(fieldError));
                            return bodyErrors;
                        },
                        new ApiError.FieldViolation("paymentMethod", "must match \"CASH|CARD\"")
                )
        );
    }
