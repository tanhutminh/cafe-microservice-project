...
/**
 * @WebMvcTest slice test - the template for this codebase's first MockMvc-based controller
 * coverage (see the other 6 REST controllers for the same pattern applied). Unlike the plain
 * unit tests elsewhere in this codebase, this actually drives requests through Spring MVC's
 * real argument-resolution/validation pipeline, so it verifies what a hand-rolled
 * GlobalExceptionHandlerTest can only simulate: that @Positive/@Valid annotations on the
 * controller itself actually get enforced. @Import(SecurityConfig.class) pulls in the real
 * SecurityFilterChain + HeaderAuthenticationFilter (no @WithMockUser - this project's auth is
 * custom X-Username/X-User-Role headers, not Spring Security's own user store).
 */
...
class OrderControllerTest {
    ...
    @MockitoBean
    private OrderCheckoutSaga orderCheckoutSaga;
    ...
    /**
     * One case per distinct constraint reachable through OrderController - not just a
     * representative subset - covering every request-body DTO it validates
     * (CreateOrderRequest/AddOrderItemRequest/UpdateOrderItemQuantityRequest/MoveTableRequest/
     * PayRequest) plus the itemId path variable, which removeItem/updateItemQuantity are the
     * only endpoints validating alongside a sibling id param.
     */
    private static Stream<Arguments> validationFailures() {
        return Stream.of(
                Arguments.of("getOrder_negativeId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> get("/api/orders/-1"),
                        "id", "must be greater than 0"),
                Arguments.of("createOrder_missingTableId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders")
                                .contentType(MediaType.APPLICATION_JSON).content("{}"),
                        "tableId", "must not be null"),
                Arguments.of("createOrder_negativeTableId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"tableId\":-1}"),
                        "tableId", "must be greater than 0"),
                Arguments.of("addItem_missingMenuItemId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/items")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"quantity\":2}"),
                        "menuItemId", "must not be null"),
                Arguments.of("addItem_negativeMenuItemId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/items")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"menuItemId\":-1,\"quantity\":2}"),
                        "menuItemId", "must be greater than 0"),
                Arguments.of("addItem_zeroQuantity",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/items")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"menuItemId\":12,\"quantity\":0}"),
                        "quantity", "must be greater than or equal to 1"),
                Arguments.of("removeItem_negativeItemId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> delete("/api/orders/101/items/-1"),
                        "itemId", "must be greater than 0"),
                Arguments.of("updateItemQuantity_zeroQuantity",
                        (Supplier<MockHttpServletRequestBuilder>) () -> patch("/api/orders/101/items/42")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"quantity\":0}"),
                        "quantity", "must be greater than or equal to 1"),
                Arguments.of("moveTable_missingTableId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/move")
                                .contentType(MediaType.APPLICATION_JSON).content("{}"),
                        "tableId", "must not be null"),
                Arguments.of("moveTable_negativeTableId",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/move")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"tableId\":-1}"),
                        "tableId", "must be greater than 0"),
                Arguments.of("pay_missingPaymentMethod",
                        ...
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/pay")
                                .contentType(MediaType.APPLICATION_JSON).content("{}"),
                        "paymentMethod", "must not be blank"),
                Arguments.of("pay_invalidPaymentMethodOnValidOrder",
                        ...
                        "paymentMethod", "must match \"CASH|CARD\"")
        );
    }
    ...
    void getOrder_nonConvertibleId_returns400NotUnexpected500(String caseName, String idSegment) throws Exception {
        ...
        mockMvc.perform(get("/api/orders/" + idSegment)
                        .header("X-Username", "admin")
                        .header("X-User-Role", "ADMIN"))
                .andExpect(status().isBadRequest())
    ...
    void cancel_negativeId_returns400BeforeReachingSaga() throws Exception {
        ...
        verify(orderCheckoutSaga, never()).cancelOrder(anyLong());
    }

    private static Stream<Arguments> happyPathRequests() {
        return Stream.of(
                Arguments.of("getOrder",
                        (Supplier<MockHttpServletRequestBuilder>) () -> get("/api/orders/101"), HttpStatus.OK),
                Arguments.of("getCurrentOrderForTable",
                        (Supplier<MockHttpServletRequestBuilder>) () -> get("/api/orders").param("tableId", "3"), HttpStatus.OK),
                Arguments.of("createOrder",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"tableId\":3}"),
                        HttpStatus.CREATED),
                Arguments.of("addItem",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/items")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"menuItemId\":12,\"quantity\":2}"),
                        HttpStatus.OK),
                Arguments.of("removeItem",
                        (Supplier<MockHttpServletRequestBuilder>) () -> delete("/api/orders/101/items/42"), HttpStatus.OK),
                Arguments.of("updateItemQuantity",
                        (Supplier<MockHttpServletRequestBuilder>) () -> patch("/api/orders/101/items/42")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"quantity\":3}"),
                        HttpStatus.OK),
                Arguments.of("moveTable",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/move")
                                .contentType(MediaType.APPLICATION_JSON).content("{\"tableId\":5}"),
                        HttpStatus.OK),
                Arguments.of("checkout",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/checkout"), HttpStatus.ACCEPTED),
                Arguments.of("cancel",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/cancel"), HttpStatus.OK),
                Arguments.of("pay",
                        (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/orders/101/pay")
    ...
    void validRequest_returnsExpectedStatusWithOrder(
        ...
        switch (caseName) {
            ...
            case "getCurrentOrderForTable" -> when(orderService.getCurrentOrderForTable(3L)).thenReturn(sampleOrder());
            ...
            case "updateItemQuantity" -> when(orderService.updateItemQuantity(101L, 42L, 3)).thenReturn(sampleOrder());
            case "moveTable" -> when(orderService.moveTable(101L, 5L)).thenReturn(sampleOrder());
            case "checkout" -> when(orderCheckoutSaga.startCheckout(101L)).thenReturn(sampleOrder());
            case "cancel" -> when(orderCheckoutSaga.cancelOrder(101L)).thenReturn(sampleOrder());
            case "pay" -> when(orderCheckoutSaga.startPayment(101L, "CASH")).thenReturn(sampleOrder());
        }
