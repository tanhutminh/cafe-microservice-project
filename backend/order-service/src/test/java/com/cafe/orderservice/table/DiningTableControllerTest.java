...
class DiningTableControllerTest {
  ...
  private static final String VALID_BODY =
      "{\"tableNumber\":\"T3\",\"capacity\":4,\"active\":true}";

  @Autowired private MockMvc mockMvc;

  @Autowired private ObjectMapper objectMapper;

  @MockitoBean private DiningTableService diningTableService;
  ...
  /**
   * One case per constraint on DiningTableRequest (tableNumber/capacity) plus @Positive on every
   * endpoint's id path variable - not just a representative subset.
   */
  private static Stream<Arguments> validationFailures() {
    return Stream.of(
        ...
        Arguments.of(
            "update_missingTableNumber",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    put("/api/tables/3")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"capacity\":4,\"active\":true}"),
            "tableNumber",
            "must not be blank"),
        Arguments.of(
            "update_zeroCapacity",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    put("/api/tables/3")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"tableNumber\":\"T3\",\"capacity\":0,\"active\":true}"),
            "capacity",
        ...
        Arguments.of(
            "occupy_negativeId",
            (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/tables/-1/occupy"),
            "id",
            "must be greater than 0"),
        Arguments.of(
            "create_missingTableNumber",
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    post("/api/tables")
                        .contentType(MediaType.APPLICATION_JSON)
  ...
  void delete_nonConvertibleId_returns400NotUnexpected500(String caseName, String idSegment)
      throws Exception {
  ...
  void delete_validId_returns200() throws Exception {
    mockMvc
        .perform(
            delete("/api/tables/3").header("X-Username", "admin").header("X-User-Role", "ADMIN"))
        .andExpect(status().isOk());
  }

  @Test
  void release_missingAuth_returns401() throws Exception {
    mockMvc.perform(post("/api/tables/3/release")).andExpect(status().isUnauthorized());
  }
  ...
  void findAll_returns200WithTableList() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(
                get("/api/tables").header("X-Username", "admin").header("X-User-Role", "CASHIER"))
            .andExpect(status().isOk())
            .andReturn();
  ...
  private static Stream<Arguments> happyPathRequests() {
    return Stream.of(
        Arguments.of(
            ...
            (Supplier<MockHttpServletRequestBuilder>)
                () ->
                    post("/api/tables").contentType(MediaType.APPLICATION_JSON).content(VALID_BODY),
            HttpStatus.CREATED),
        Arguments.of(
            "update",
            (Supplier<MockHttpServletRequestBuilder>)
        ...
        Arguments.of(
            "release",
            (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/tables/3/release"),
            HttpStatus.OK),
        Arguments.of(
            "occupy",
            (Supplier<MockHttpServletRequestBuilder>) () -> post("/api/tables/3/occupy"),
            HttpStatus.OK));
  }
  ...
  void validRequest_returnsExpectedStatusWithTable(
    ...
    switch (caseName) {
      ...
      case "occupy" -> when(diningTableService.findById(3L)).thenReturn(sampleTable());
    }
