...
public class OrderSagaStateService {
   ...
   * start(), which inserts a new row, this order already has one from the checkout leg. Fresh
   * correlationId (same reasoning as start()'s) and a reset retry count, since this is a new
   * attempt distinct from the checkout leg's retries.
   */
  @Transactional
  ...
  /**
   * Guards against two distinct kinds of stale replies: Kafka redelivering a reply after the saga
   * already reached a terminal step, and a reply for an attempt that's no longer current (the order
   * was checked out again — e.g. after a prior failure — before this one arrived).
   *
   * <p>CONFIRMED counts as terminal here too, alongside COMPLETED/COMPENSATED, even though it isn't
   ...
   * CONFIRMED, this guard needs revisiting.
   */
  @Transactional(readOnly = true)
  public boolean shouldIgnoreReply(Long orderId, String correlationId) {
    return sagaStateRepository
        ...
        .map(
            state ->
                state.getStep() == SagaStep.COMPLETED
