...
/**
 * {@link ObservationPredicate} logic for excluding a service's own recurring {@code @Scheduled}
 * pollers from tracing. Kept as a plain static factory rather than an auto-configured bean so the
 * owning config class still declares its own {@code @Bean} explicitly - this is an internal
 * monorepo, not a published starter for external consumers.
 */
final class ScheduledPollerObservationPredicates {
   ...
   */
  static ObservationPredicate excludingTargets(Set<Class<?>> excludedTargets) {
    return (name, context) ->
